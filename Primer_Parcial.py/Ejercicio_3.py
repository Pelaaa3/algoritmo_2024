# 3. Se dispone de una lista de todos los Jedi, de cada uno de estos se conoce su nombre, maestros, colores de sable de luz usados y especie. 
...
ordenada = sorted(jedis, key=by_name)
jedis_cola = Queue()

for jedi in ordenada:
  jedis_cola.arrive(jedi)
  
print("Lista ordenada segun su nombre y especie:")
...
for jedi in range (len(jedis)):
    if jedis[jedi]["master"] is not None:
        if "Luke Skywalker" in jedis[jedi]["master"] or "Yoda" in jedis[jedi]["master"]:
...
for jedi in jedis:
  if jedi["lightsaber_color"] and "/" in jedi["lightsaber_color"]:
    Sable_de_luz.append(jedi["name"])

print("Los jedis que usaron sables de luces de colores distintos son:")
for name in Sable_de_luz:
  print(name)
